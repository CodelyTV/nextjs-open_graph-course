...
export async function GET(request: Request) {
	try {
		...
		const color = searchParams.has("color")
			? hexColor(searchParams.get("color") as string)
			: "#3cff64";
		const sticker = searchParams.has("sticker")
			? searchParams.get("sticker")
			: "/img/cursos/eventos-dominio.png";
		const authors = searchParams.has("authors")
			? (searchParams.get("authors")?.split(",") as string[])
			: [];
			...
			(
				...
				>
					...
					<div style={{ display: "flex", width: "70%", padding: "4rem", position: "relative" }}>
						<h1 style={{ fontSize: "4rem", margin: "0" }}>{text}</h1>
